...
export class AudioEngine {
  ...
  buildGraph() {
    ...
    if (p.distOn && distNode) {
      effectsInput.connect(distDryGain); distDryGain.connect(effectsMix);
    ...
    if (p.chorusOn && chorusDelay) {
      effectsInput.connect(chorusDryGain); chorusDryGain.connect(effectsMix);
  ...
  async export(options, onProgress) {
    ...
    const revDry = octx.createGain(); revDry.gain.value = 1;
    ...
    src.connect(chain[0]);
    // Reverb parallel
    masterGain.connect(revDry); revDry.connect(octx.destination);
    if (p.reverbOn) {
      masterGain.connect(reverbNode); reverbNode.connect(revWet); revWet.connect(octx.destination);
    }
    src.start(0);
